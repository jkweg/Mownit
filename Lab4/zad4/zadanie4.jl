...
sr_budowa_n = Float64[]; odch_budowa_n = Float64[]
sr_ewal_n = Float64[];   odch_ewal_n = Float64[]

sr_budowa_p = Float64[]; odch_budowa_p = Float64[]
sr_ewal_p = Float64[];   odch_ewal_p = Float64[]

println("Rozpoczynam pomiary... to może chwilę potrwać.")
...
for n in rozmiary
    ...
    push!(sr_budowa_n, mean(czasy_bud_n)); push!(odch_budowa_n, std(czasy_bud_n))
    push!(sr_ewal_n, mean(czasy_ewal_n));  push!(odch_ewal_n, std(czasy_ewal_n))
    
    push!(sr_budowa_p, mean(czasy_bud_p)); push!(odch_budowa_p, std(czasy_bud_p))
    push!(sr_ewal_p, mean(czasy_ewal_p));  push!(odch_ewal_p, std(czasy_ewal_p))
end

p1 = plot(rozmiary, sr_budowa_n, yerr=odch_budowa_n, label="Newton", marker=:circle, 
          title="Czas TWORZENIA wielomianu", xlabel="Liczba węzłów (n)", ylabel="Czas [s]", color=:green)
plot!(p1, rozmiary, sr_budowa_p, yerr=odch_budowa_p, label="Polynomials", marker=:square, color=:blue)

p2 = plot(rozmiary, sr_ewal_n, yerr=odch_ewal_n, label="Newton", marker=:circle, 
          title="Czas EWALUACJI w punkcie", xlabel="Liczba węzłów (n)", ylabel="Czas [s]", color=:green)
...
savefig("zadanie4_pomiary_czasu.png")